...
In Python, the following code would implement the network with backpropagation using the same notational logic as we've used before. First, we define the MLP object and forward propagation, now saving all the activations and raw_inputs:
...
def forward_propagate(self, inputs):
    ...
    for i, w in enumerate(self.weights):
        ...
        self.net_inputs[i + 1] = net_inputs

        # return output layer activation
        return activations
```

Then, we define backward propagation as defined previously:
